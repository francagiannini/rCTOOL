...
Where only daily data are available (min and max temperature), rCTOOL provides built-in functions to seamlessly convert into monthly values.
```{r}
...
ggplot(df, aes(x=date, y=(Tmax+Tmin)/2, colour=(Tmax+Tmin)/2)) + 
  geom_point(size=.3, alpha=.5) +
  scale_colour_viridis_c(option = 'E') + 
  theme_bw() + 
  labs(x='Timeperiod', y='Average daily temperature (ºC)', colour='') + 
  theme(text = element_text(size=16, family='serif'),
      axis.title = element_text(size=17, face='bold'),
      axis.text = element_text(size=16))
```
